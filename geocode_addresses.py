...
def parse_arguments():
    ...
    parser.add_argument("--output_name", default="geocoded_final", help="Base name for outputs")
    parser.add_argument("--output_folder", default="outputs", help="Folder for outputs")
    parser.add_argument("--formats", default="GPKG", help="Comma-separated list of spatial formats (GPKG,SHP)")
...
def run_validation_pipeline(df):
    ...
    df["ORIGINAL_ID"] = df["ID"] if "ID" in df.columns else None
    ...
    # Reset index for consistent iteration/logging
...
def process_geocoding(df, geocode_func, partial_path):
    ...
    for idx, row in df.iterrows():
        ...
        # Save partial progress every 10 rows
...
def create_geodataframe(df):
    ...
    gdf = gpd.GeoDataFrame(df.reset_index(drop=True), geometry="geometry", crs="EPSG:4326")
    return gdf
...
def main():
    ...
    # Validation stage only if starting fresh
    ...
    # Export stage
    save_outputs(df, gdf, args.output_folder, args.output_name, formats)
